<?php
	...
	//$dbname_2 = array('hamko_hrm','asset_mgmt','banner','materils');
	$dbname_2 = array('hamko_hrm');
	for($j=0;$j<count($dbname_2);$j++){
		$miss_table_db = $dbname_2[$j];
		$db_2 = array();
		unset($db_2);
		...
		$k = 1;
		if(!empty($db_2_miss)){?>
			<h4 style ="color:green"><?php echo "Missing table List in database ->".$miss_table_db;?></h4><?php
		...
		$l = 1;
		if(!empty($db_2_extra)){ ?>
			<h4 style ="color:green"><?php echo "Extra table List in database ->".$miss_table_db;?></h4><?php
		...
		if(!empty($common_table)){
			...
			if(!empty($filter_table)){
				...
				for($i=0;$i<count($filter_table);$i++){
					...
					$sql = "SELECT CONCAT(COLUMN_NAME,'-',COLUMN_TYPE) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '$dbname_2[$j]' AND TABLE_NAME = '$filter_table[$i]' ";
					$statement = $db->prepare($sql);
					$statement->execute();
					...
					*/
					...
					if(count($column_match_with)==count($column_match) && count($column_match_with)==count($result_column_match)){
						...
						if(!empty($column_miss)){
							?><h4 style="color:blue"><?php echo $miss_table_db.' -> '.$filter_table[$i].' -> '.' column name missing or change <br>';?></h4><?php
							foreach ($column_miss as $value) {
								...
								if(strpos(implode(' ', $column_match), $val[0]) !== false){
									echo 'ALTER TABLE `'.$filter_table[$i].'` CHANGE `'.$val[0].'` `'.$val[0].'` '.$val[1].';'.'<br>';
								}else{
									echo 'ALTER TABLE `'.$filter_table[$i].'` ADD `'.$val[0].'` '.$val[1].';'.'<br>';
								}
						...
						if(!empty($column_extra)){
							foreach ($column_extra as $extra) {
								...
								if(strpos(implode(' ', $column_match_with), $extra_val[0]) !== false){
									...
									echo 'ALTER TABLE `'.$filter_table[$i].'` DROP `'.$extra_val[0].'`;'.'<br>';
								}
							}
						}
					}
